...
namespace GearFramework.Common.EditorOnly
{
    public class PathUtil
    {
        ...
        public static string GetAssetFilePath_ForInstances(Type type, string searchDirectoryPath)
        {
            ...
            foreach (string item in allfiles)
            {
                if (item.Contains(namespaceToPathPart))
                    return GetAssetFilePath_ForInstances(item, Settings.Project.Root);
            }
            ...
            if (allfiles.Length > 0)
            {
                ...
                return GetAssetFilePath_ForInstances(allfiles[0], Settings.Project.Root);
            }
        ...
        public static string PathModifyByNamespace(Type type, string searchDirectoryPath)
        {
            string[] path = type.Namespace.Split('.');
            path[0] = NavigateBackToTheDirectory(searchDirectoryPath, Settings.Project.Root);
